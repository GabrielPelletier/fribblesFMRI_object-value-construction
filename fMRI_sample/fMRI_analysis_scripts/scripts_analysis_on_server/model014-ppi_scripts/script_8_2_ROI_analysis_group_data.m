...
% Model
modelID = 'model014';
modelFullName = 'model014-ppi-model013_cope6_vmpfc';
...
for roi_ind = 1:length(rois)
    ...
    for cope_ind = 1:length(copes)
        ...
        if ~exist([main_path, 'group/model/' modelFullName],'dir') mkdir([main_path, 'group/model/' modelFullName]); end
        
        groupFileDir = [main_path, 'group/model/' modelFullName '/ROI_analysis/'];
        ...
        for sub_ind = 1:length(participants)
            ...
            reportFile = [main_path 'sub-' SUBNUM '/model/' modelFullName '/task-fribBids.gfeat/cope' COPENUM '.feat/featquery_roi-' ROINAME '/report.txt'];

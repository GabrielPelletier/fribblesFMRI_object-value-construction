    ...
    for task_ind = 1: length(tasknames)
        ...
        for seed_ind = 1 : length(seeds)
            ...
            for run_ind = whichRuns
                ...
                fsf_name=['design_sub-',SUBNUM,'_task-',TASKNAME,'_ppi_seed-' SEEDNAME '_run-',RUNNUM,'.fsf'];

                template_in = fopen([main_path,'scripts/' modelID '_scripts/lvl1_task-',TASKNAME,'_ppi_template.fsf']);

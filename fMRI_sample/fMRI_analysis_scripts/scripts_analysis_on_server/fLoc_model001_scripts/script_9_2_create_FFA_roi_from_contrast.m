...
%
% It thresholds the zmap to an uncorrected value (z = 3.1), finds
% the peak within and atlas-based region, and draws a sphere around the
% peak.
% 
% Creates one ROI per hemisphere, and also creates a bilateral ROI by
% combining the left and right ROIs.
%
% The scripts is written for the Fusiform Face Area (FFA), based on the
% Hardvardoxford atlas and the Face > Objects contrast. It can
...
for sub_ind = 1:length(participants)
    ...
    % Step 1. Threshold the z-map and create a new file.
     THRESHOLD = '3.1';
     % Input STATISTICAL MAP that correpsonds to the CONTRAST of interes
     INPUTimage = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/zstat1.nii.gz'];
     STATSimage= [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/zstat1_thresholded.nii.gz'];
        % Command
        system(['fslmaths ' INPUTimage ' -thr ' THRESHOLD ' ' STATSimage]);

    % Step 2.1 Tranform the Atlas-based ROI to fit the subject's registered stats image (dimensions and Voxel size)
     INPUTimage = [mainPATH '/ROI_standard_masks/harvardoxford_TempOccFusiCortex_bin.nii.gz']; % The Atlas ROI image to resample (transform)
     OUTPUTimage = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/harvardoxford-TempOccFusiCortex_bin_mask.nii.gz'];
        % Command
    ...
    % Output name:
    atlas_derived_FFA = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/harvardoxford_TempOccFusiCortex_mask.nii.gz'];
        % Command
         system(['fslmaths ' your_output_from_the_previous_command ' -thr 0.5 -bin ' atlas_derived_FFA]);
    
    
    % Step 3. Create an image which is a conjunction between the atlas
    % Fusiform gyrus and the thresholded zmap. (multiply the thresholded image by the
    % binary atlas-derived FFA).
     temp_output = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/zstat1_conj_FFA.nii.gz'];

     % Multiply thresholded image by the binarized Atlas FFA
        % Command
        system(['fslmaths ' STATSimage ' -mul ' atlas_derived_FFA ' ' temp_output]);
    

    % Step 4. Get the clusters information within the atals-FFA mask.
     ...
     cluster_info_output = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/FFA_cluster_info.txt'];
   ...
   % Step 5. Get coordinates of the PEAK VOXEL in each Hemisphere.
    ...
    ind_peak_left = find(cluster_info.MAX == val_peak_left);
    val_peak_right = (max(cluster_info.MAX(cluster_info.MAXX_vox_ > midline)));
    ind_peak_right = find(cluster_info.MAX == val_peak_right);
   ...
   % [...]
   % Step 6. Draw a Sphere around the peaks and save as final FFA ROIs for 
    % this subject, using flsmaths
    % Save the coordinates as .nii   
    template = temp_output; % Canvas to draw the ROI is the stats image
    leftPointName = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/FFA_left_peak.nii.gz'];
    rightPointName = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope2.feat/stats/FFA_right_peak.nii.gz'];
        % Command
    ...
    leftRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-FFA_left_mask.nii.gz'];
    rightRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-FFA_right_mask.nii.gz'];
   ...
   % Combine the two ROIs in a bilateral FFA ROI
   bilatRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-FFA_mask.nii.gz'];

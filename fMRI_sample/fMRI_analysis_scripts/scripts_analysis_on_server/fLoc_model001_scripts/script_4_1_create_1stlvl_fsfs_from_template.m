...
%% Run(s)
whichRuns = [1 2 3 4]; % We happen to have 4 runs for both tasks
...
for sub_ind = 1:length(participants)
    ...
    for task_ind = 1: length(tasknames)
        ...
        for run_ind = whichRuns
            ...
            [status,info] = system(['fslinfo ' main_path 'fLoc/sub-' SUBNUM '/sub-' SUBNUM '_task-' TASKNAME '_run-0'  num2str(run_ind) '_space-MNI152NLin2009cAsym_desc-fmriprep_brain_bold.nii.gz']);
            TEMP=strsplit(info);
            NPTS=TEMP{1,10};
            
            fsf_out_dir = [main_path 'scripts/fLoc_' modelID '_scripts/fsfs/lvl1/'];
            
            fsf_name=['design_sub-' SUBNUM '_task-' TASKNAME '_run-' RUNNUM '.fsf'];
            
            template_in = fopen([main_path 'scripts/fLoc_' modelID '_scripts/lvl1_task-' TASKNAME '_template.fsf']);

...
%
% The scripts is written for the Parahippocampal Place Area (PPA), based on the
% Hardvardoxford atlas and the Places > Objects contrast. It can
...
for sub_ind = 1:length(participants)
    ...
    % Step 1. Threshold the z-map and create a new file.
     ...
     INPUTimage = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/zstat1.nii.gz'];
     STATSimage= [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/zstat1_thresholded.nii.gz'];
        % Command
    ...
    % Step 2.1 Tranform the Atlas-based ROI to fit the subject's registered stats image (dimensions and Voxel size)
     INPUTimage = [mainPATH '/ROI_standard_masks/harvardoxford_ParahippoPostAnt_bin.nii.gz']; % The Atlas ROI image to resample (transform)
     OUTPUTimage = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/harvardoxford-ParahippoPostAnt_bin_mask.nii.gz'];
        % Command
    ...
    % Output name:
    atlas_derived_PPA = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/harvardoxford_ParahippoPostAnt_mask.nii.gz'];
        % Command
         system(['fslmaths ' your_output_from_the_previous_command ' -thr 0.5 -bin ' atlas_derived_PPA]);
    
    
    % Step 3. Create an image which is a conjunction between the atlas
    % Parahippocampal gyrus and the thresholded zmap. (multiply the thresholded image by the
    % binary atlas-derived PPA).
     temp_output = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/zstat1_conj_PPA.nii.gz'];

     % Multiply thresholded image by the binarized Atlas PPA
        % Command
        system(['fslmaths ' STATSimage ' -mul ' atlas_derived_PPA ' ' temp_output]);
    

    % Step 4. Get the clusters information within the atals-PPA mask.
     ...
     cluster_info_output = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/PPA_cluster_info.txt'];
   ...
   % Step 6. Draw a Sphere around the peaks and save as final LOC ROIs for 
    ...
    leftPointName = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/PPA_left_peak.nii.gz'];
    rightPointName = [mainPATH '/fLoc/sub-' SUBNUM '/model/' modelID '/task-fLoc.gfeat/cope3.feat/stats/PPA_right_peak.nii.gz'];
        % Command
    ...
    leftRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-PPA_left_mask.nii.gz'];
    rightRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-PPA_right_mask.nii.gz'];
   ...
   % Combine the two ROIs in a bilateral PPA ROI
   bilatRoiName = [mainPATH '/fLoc/sub-' SUBNUM '/sub-' SUBNUM '_desc-PPA_mask.nii.gz'];

classdef fLocSequence
    ...
    properties
        ...
        isifix_onsets % onset times of each ISI fixation
    ...
    properties (Constant)
        stim_conds = {'Faces' 'Scenes' 'Objects' 'Scrambled'};
        %stim_conds = {'Bodies' 'Characters' 'Faces' 'Objects' 'Places'};
        stim_per_block = 15;   % number of stimuli in a block
        stim_duty_cycle = 0.750; % duration of stimulus duty cycle (s)  ISI + Stim dur       %%%% GP Jan2018
        pre_dur = 5; %  Seconds of fixation before runs starts.    
    end
    
    properties (Constant, Hidden)
        stim_set1 = {'body' 'word' 'adult' 'car' 'house'};
        stim_set2 = {'limb' 'number' 'child' 'instrument' 'corridor'};
        stim_set3 = {'face' 'scene' 'flower' 'scrambled'};
        stim_per_set = 16;
    ...
    methods
        ...
        function run_dur = get.run_dur(seq)
            ...
            blocks_per_run = (length(seq.stim_conds)) * 4 + 1;                  %%%% GP Jan 2018
        ...
        function isi_dur = get.isi_dur(seq)
            if seq.task_num == 3
                isi_dur = 0;
            elseif seq.stim_set == 4
                isi_dur = 0.450; % If we use the Custom StimSet and Task    %%%% GP Jan 2018
            else
                isi_dur = 0.1;
            end
        end
        ...
        function num_conds = get.num_conds(seq)
            num_conds = length(seq.stim_conds);
        end
        ...
        function seq = make_runs(seq)
            ...
            block_conds = [block_conds; zeros(1, seq.num_runs)]; % Baseline Cond at end of each run  %%%% GP March 2018
            block_dur = seq.stim_per_block * seq.stim_duty_cycle;
            ...
            stim_mat = cell(seq.stim_per_block, seq.num_conds ^ 2 + 1, seq.num_runs);                  %%%% GP Jan 2018
            for rr = 1:seq.num_runs
                cat_list = ['baseline' seq.run_sets(rr, :)];
                %cat_list = seq.run_sets(rr, :);                                                       %%%% GP Jan 2018
                cat_seq = cat_list(block_conds(:, rr) + 1);
                %cat_seq = cat_list(block_conds(:, rr));                                               %%%% GP Jan 2018
            ...
            probe_stim_mat = zeros(seq.stim_per_block, seq.num_conds ^ 2 + 1, seq.num_runs);   %%%% GP Jan 2018
            for rr = 1:seq.num_runs
            ...
            stim_onsets = stim_onsets + seq.pre_dur;
            isifix_onsets = stim_onsets + seq.stim_dur;                                                 %%%% GP Jan 2018
            ...
            seq.isifix_onsets = isifix_onsets;                                                         %%%% GP Jan 2018

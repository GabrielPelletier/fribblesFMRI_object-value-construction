...
% 
% 2. Make the starting position not fixed, but random (code around line 180).


function [position, RT, answer, t0, startingPosition, sliderPath] = slideScale_mod(screenPointer, question, rect, endPoints, anchorValues, varargin)
...
displayPos    = false;
sliderSpeed = 9; % Only in use if devide = keyboard and use arrowKeys to slide
...
% Declare variables GP
sliderPath = []; % Will hold the full path of the slider during the trial. GP

% Set keys depending on Keyboard or Scanner Response box
...
% position
scaleRange        = round(rect(3)*(1-scalaLength)):round(rect(3)*scalaLength); % Calculates the range of the scale (in pixels)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Screen('TextSize', screenPointer, 35);%25
    ...
    % Save the position to the matrix containing the full slider path. GP
    sliderPath(end+1, 1) = position;
...
% Calculate starting position of the scale, important if set at Random.
 startingPosition = round (startingPosition-scaleRange(1));
...
RT = (secs - t0) * 1000;% converting RT to millisecond
    %  dummy rt if mAX rt exceeded
    try
        if abort == 1
                RT = -999;
        end
    catch
        disp('error in setting negative RT for missed, not critical');
    end
end

%% This script was modified by Gabriel Pelletier to fit a particular task,
%  in January 2018. It is much less flexible than the original script.
%
%   !!    It is better to use slideScale_originalScript.m   !! %


function [position, RT, answer, t0] = slideScale_learn(screenPointer, question, rect, endPoints, anchorValues, rectImage, varargin)
...
elseif strcmp(startPosition, 'center')
    x = (rectImage(1) + (rectImage(3)-rectImage(1))/2);
...
% leftTick   = [rectImage(1)*(1-scalaLength) rect(4)*scalaPosition - lineLength rectImage(1)*(1-scalaLength) rect(4)*scalaPosition  + lineLength];
% rightTick  = [rectImage(3)*scalaLength rect(4)*scalaPosition - lineLength rectImage(3)*scalaLength rect(4)*scalaPosition  + lineLength];
...
% Added by GP, Jan 2018
% Use the image x-size and position to draw the scale.
midTick    = [(rectImage(1) + (rectImage(3)-rectImage(1))/2) rect(4)*scalaPosition - lineLength - 5 (rectImage(1) + (rectImage(3)-rectImage(1))/2) rect(4)*scalaPosition  + lineLength + 5];
leftTick   = [rectImage(1)*scalaLength rect(4)*scalaPosition - lineLength rectImage(1)*scalaLength rect(4)*scalaPosition  + lineLength];
rightTick  = [rectImage(3)*scalaLength rect(4)*scalaPosition - lineLength rectImage(3)*scalaLength rect(4)*scalaPosition  + lineLength];
horzLine   = [rectImage(1)*scalaLength rect(4)*scalaPosition rectImage(3)*scalaLength rect(4)*scalaPosition];
textBounds = [Screen('TextBounds', screenPointer, endPoints{1}); Screen('TextBounds', screenPointer, endPoints{2})];
...
% position
scaleRange        = round(rectImage(1)):round(rectImage(3)); % Calculates the range of the scale in pixels
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if drawImage == 1
         Screen('DrawTexture', screenPointer, stimuli, [] , [], 0);
    end
    
    % Drawing the question as text
    DrawFormattedText(screenPointer, question, rectImage(1)-20, rect(4)*(scalaPosition - 0.1),textColor); 
    ...
    if displayPos
        DrawFormattedText(screenPointer, num2str(round(position)), x-(bounds(1,3)/2), rect(4)*(scalaPosition + 0.02), [125 125 125]);

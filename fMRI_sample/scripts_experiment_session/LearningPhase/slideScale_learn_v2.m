function [position, RT, answer, t0] = slideScale_learn_v2(screenPointer, question, rect, endPoints, anchorValues, stimulusValue, stimAveValue, varargin)
...
displayPos    = false;
sliderSpeed = 6; % Only in use if devide = keyboard and use arrowKeys to slide
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if drawImage == 1
         Screen('DrawTexture', screenPointer, stimuli,[] , imageRect, 0);
    end
...
% FeedBack Slider (Real value for this trial)
...
if displayPos
    unicodeShekel = 8362;
    fbtext = double([unicodeShekel,uint16(num2str(round(stimulusValue)))]);
        DrawFormattedText(screenPointer, fbtext, feedbackPos -(bounds1(1,3)/2), rect(4)*(scalaPosition) - 50, [255 255 51]);
end

...
function [groupEyeDerivatives] = level1_analysis (subs, which_runs, main_path)
...
for subjectNumber = subs
    ...
    if isfile([bids_dataPath subID '/' num2str(subjectNumber) '_eyeData_run1.mat'])
        ...
        for run = which_runs
            ...
            temp_matrix = sortrows([trans_conj; trans_summ]);
            ...
            temp_meanDur = sortrows([durFix_conj; durFix_summ]);
            temp_matrix(:,5) = temp_meanDur(:,2);
            
            %% Average and Load data in matrices that will hold the subject-level means
            groupTrans_conj(end+1, 1) = nanmean(trans_conj(:,2));

            groupTrans_summ(end+1, 1) = nanmean(trans_summ(:,2));

            groupGazeDur_conj(end+1, 1) = nanmean(gazeDur_conj(:,2)); % ROI-1
            groupGazeDur_conj(end, 2) = nanmean(gazeDur_conj(:,3)); % ROI-2
            groupGazeDur_conj(end, 3) = nanmean(gazeDur_conj(:,4)); % No ROI
            groupGazeDur_conj(end, 4) = nanmean(gazeDur_conj(:,5)); % Symmetric viewing index

            groupGazeDur_summ(end+1, 1) = nanmean(gazeDur_summ(:,2)); % ROI-1
            groupGazeDur_summ(end, 2) = nanmean(gazeDur_summ(:,3)); % ROI-2
            groupGazeDur_summ(end, 3) = nanmean(gazeDur_summ(:,4)); % No ROI
            groupGazeDur_summ(end, 4) = nanmean(gazeDur_summ(:,5)); % Symmetric viewing index

            groupNumFix_conj (end+1,1) = nanmean(numFix_conj(:,2));

            groupNumFix_summ (end+1,1) = nanmean(numFix_summ(:,2));

            groupDurFix_conj (end+1,1) = nanmean(durFix_conj(:,2));

            groupDurFix_summ (end+1,1) = nanmean(durFix_summ(:,2));
            ...
            % save1
            eyeData.trial_fixation_summary = array2table(temp_matrix, 'VariableNames', {'trial','num_transitions','num_fixations', 'value', 'average_fixation_duration'});
...
groupEyeDerivatives.TransPerTrial_conj = groupTrans_conj;
groupEyeDerivatives.TransPerTrial_summ = groupTrans_summ;
groupEyeDerivatives.NumFixPerTrial_conj = groupNumFix_conj;
groupEyeDerivatives.NumFixPerTrial_summ = groupNumFix_summ;
groupEyeDerivatives.AveFixDur_conj = groupDurFix_conj;
groupEyeDerivatives.AveFixDur_summ = groupDurFix_summ;
